...
for gen in "${genes[@]}"; do # Loop through the genes
    ...
    single_line_fa "${file_sequences}" # Function taken from Scripts/functions_bash.sh
    ...
    for spp in ${spp_tr[@]} ; do
        ...
        # The function group_best_cov is taken from Scripts/functions_bash.sh.

    done
        
        # Align nucleotides, translate and align amino acids
        aln_trn ${gen} ${file_seq} ${thr} # seqkit v2.8.2. mafft v7.526. The function aln_trn is taken from Scripts/functions_bash.sh.
